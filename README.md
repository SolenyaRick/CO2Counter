...
- **Home** — the landing page. The very first thing on it is a compact
  ...
  container gives it. Both slides are also kept the same height as each
  other (`renderPeriodChart()` in `app.js`): after the treemap slide
  renders, if it comes out taller than the chart slide (typically it
  does, once its color key wraps across a few rows), the chart is
  re-rendered a second time with its SVG grown by exactly that
  difference, so the extra room goes into a visibly bigger chart rather
  than sitting empty underneath a shorter one - legend height doesn't
  depend on the SVG's own height, so one extra pass lands on an exact
  match. `syncCarouselHeight()` then sets the carousel's own height to
  match whichever slide is actually visible (called after every re-render
  and on every swipe) rather than relying on the native flex row's
  default behavior of stretching every slide to match whichever one has
  the most content, which is what produced the mismatch in the first
  place. Slide one is the budget-pace chart
